...
_VALID_TYPES = frozenset({"DWG", "DXF", "DXF_B", "DXF_A"})


def _decode_stream(stream: bytes) -> str:
    """解码 ODA 输出流：中文 Windows 下 ODA 常输出 GBK。

    先按 UTF-8 解码，失败再回退 GBK。
    """
    text = None
...
class OdaConverter:
    ...
    def convert_batch(
        ...
        if out_version not in _VALID_VERSIONS:
        ...
        if proc.returncode not in (0, None):
            ...
            for stream, name in ((proc.stdout, "stdout"), (proc.stderr, "stderr")):
                if stream:
                    tail = _decode_stream(stream).strip()[-500:]
                    if tail:
                        detail += f"\n  {name}: {tail}"
